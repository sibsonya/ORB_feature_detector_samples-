...
int matching(Mat img_1, Mat img_2, vector<KeyPoint> keypoints_1, Mat descriptors_1, map<pair<float, float>, pair<int, Mat>> &points) {
	...
	imwrite("salt_and_pepper.jpg", img_matches);
...
int main() {
	...
	vector<int> coeff_black = { 5, 10, 15, 20, 25, 30 };
	vector<int> coeff_white = { 250, 245, 240, 235, 230, 225 };
	for (int i = 0; i != 6; ++i) {
		Mat black = uniform_noise < coeff_black[i];
		Mat white = uniform_noise > coeff_white[i];
		Mat noisy_image = img.clone();
		noisy_image.setTo(255, white);
		noisy_image.setTo(0, black);
